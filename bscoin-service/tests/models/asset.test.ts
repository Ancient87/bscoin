...
import { testTwoHundredDollarToken, testExchange, testAsset, testStableToken } from "./fixtures/fixtures";
...
describe("Test totalValue", () => {
    ...
    itParam(`should compute correct totalValue ${testCases}`, testCases, (testCase: { config: IAssetConfig; expected: any; }) => {
...
describe("Test asset share for hypothetical added liquidity", () => {
    ...
    itParam(`should compute correct hypotheticalshare ${testCases}`, testCases, (testCase: { config: IAssetConfig; args: number[]; expected: any; }) => {
        const assetUnderTest = new Asset({
            ...testCase.config
        });

        expect(assetUnderTest.futureAssetShareForDollarAmount(testCase.args[0])).to.equal(testCase.expected);
    });
});